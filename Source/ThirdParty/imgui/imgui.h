...
namespace ImGui
{
    ...
    IMGUI_API bool          BeginPopupContextItem(const char* str_id, int mouse_button = 2);                                        // helper to open and begin popup when clicked on last item. read comments in .cpp!
    IMGUI_API bool          BeginPopupContextWindow(const char* str_id = NULL, int mouse_button = 2, bool also_over_items = true);  // helper to open and begin popup when clicked on current window.
    IMGUI_API bool          BeginPopupContextVoid(const char* str_id = NULL, int mouse_button = 2);                                 // helper to open and begin popup when clicked in void (no window).
...
struct ImGuiIO
{
    ...
    bool        MouseDown[5];               // Mouse buttons: left, middle, right + extras. ImGui itself mostly only uses left button (BeginPopupContext** are using right button). Others buttons allows us to track if the mouse is being used by your application + available to user as a convenience via IsMouse** API.
